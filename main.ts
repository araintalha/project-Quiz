...
let user: { user1: string } = await inquirer.prompt({
    type: "input",
    name: "user1",
...
let doneArr:string[]=[];
let isQuiz1:boolean=false;
let isQuiz2:boolean=false;
let score: number = 0;
let scoreQuiz2:number=0;
let Quizs: { quiz: string };
let exit:{quit:string};
do{
...
if (Quizs.quiz === "quiz1") {
  ...
  if(!isQuiz1){
    ...
    let quesQuiz1:{ques1:string} = await inquirer.prompt({
        type: "list",
        name: "ques1",
    ...
    let quesQuiz2:{ques2:string}=await inquirer.prompt(
    {
        type: "list",
        name: "ques2",
...
let quesQuiz3:{ques3:string}=await inquirer.prompt({
    type: "list",
    name: "ques3",
...
let quesQuiz4:{ques4:string}=await inquirer.prompt({
    type: "list",
    name: "ques4",
